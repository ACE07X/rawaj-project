...
import {
    LogOut, Building2, Settings, PlusCircle,
    Trash2, Edit, MapPin, DollarSign, LayoutGrid, X, Loader2, Code2, Upload, Image as ImageIcon, CheckCircle
} from 'lucide-react';
...
const Admin = () => {
    ...
    const handleAddProperty = async (e) => {
        ...
        try {
            ...
            let error;
            if (editingId) {
                const { error: updateError } = await supabase.from('properties').update(propertyData).eq('id', editingId);
                error = updateError;
            } else {
        ...
        <div className="min-h-screen bg-slate-950 text-white flex">
            ...
            <aside className="w-64 bg-slate-900 border-r border-white/5 flex flex-col hidden md:flex">
                ...
                <nav className="flex-1 p-4 space-y-2">
                    <div className="w-full flex items-center gap-3 px-4 py-3 rounded-xl font-medium bg-primary-500/10 text-primary-400">
                ...
                <div className="p-4 md:p-8 pb-32">
                    ...
                    <div className="glass-card rounded-2xl overflow-hidden">
                        ...
                        {loading ? (
                            ...
                            <div className="p-12 text-center text-slate-400">No properties found.</div>
                        ) : (
                            <div className="overflow-x-auto">
                                <table className="w-full text-left">
                                    <thead className="bg-slate-800/50">
                                        <tr>
                                            <th className="px-6 py-4 text-slate-400 font-medium text-sm">Property</th>
                                            <th className="px-6 py-4 text-slate-400 font-medium text-sm">Type</th>
                                            ...
                                            <tr key={property.id} className="hover:bg-white/5 transition-colors">
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            <div className="font-medium line-clamp-1 text-sm">{property.title_en || property.title}</div>
                                                ...
                                                <td className="px-6 py-4 text-slate-300 capitalize text-sm">{property.type}</td>
                                                <td className="px-6 py-4">
                                                    <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${property.status === 'sold' ? 'bg-red-500/20 text-red-400' :
                                                ...
                                                <td className="px-6 py-4 font-semibold text-primary-400 text-sm">{parseInt(property.price).toLocaleString()} OMR</td>
                                                <td className="px-6 py-4 text-right">
                                                    <div className="flex items-center justify-end gap-2">
                                                        <button onClick={() => handleEditProperty(property)} className="p-2 rounded-lg text-primary-400 hover:bg-primary-500/20 transition-colors">
                    ...
                    <div className="bg-slate-900 rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto border border-white/10 shadow-2xl animate-zoom-in">
                        <div className="sticky top-0 bg-slate-900 border-b border-white/5 p-6 flex items-center justify-between z-10">
                            <h3 className="text-xl font-bold flex items-center gap-2">
                                {editingId ? <><Edit className="w-5 h-5 text-primary-500" /> Edit Property</> : <><PlusCircle className="w-5 h-5 text-primary-500" /> Post New Property</>}
                            </h3>
                        ...
                        <form onSubmit={handleAddProperty} className="p-6 space-y-6">
                                ...
                                <div className="border-2 border-dashed border-white/10 rounded-xl p-8 text-center hover:border-primary-500/50 transition-colors bg-slate-800/50">
                                    {(imageFile || newProperty.image_url) ? (
                                        <div className="relative inline-block">
                                            ...
                                            <button type="button" onClick={() => { setImageFile(null); setNewProperty({ ...newProperty, image_url: '' }); }} className="absolute -top-2 -right-2 bg-red-500 text-white p-1 rounded-full shadow-lg hover:bg-red-600"><X className="w-4 h-4" /></button>
                                        </div>
                                    ) : (
                                        <div className="flex flex-col items-center gap-2">
                                            <div className="p-4 rounded-full bg-slate-800"><ImageIcon className="w-8 h-8 text-slate-500" /></div>
                                            ...
                                            <label htmlFor="file-upload" className="cursor-pointer bg-white/10 px-4 py-2 rounded-lg hover:bg-white/20 transition-colors text-sm font-medium mt-2">Select Photo</label>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <button type="submit" disabled={submitting || uploading} className="w-full bg-primary-600 hover:bg-primary-500 text-white py-4 rounded-xl font-bold transition-colors disabled:opacity-50 flex items-center justify-center gap-2 text-lg shadow-lg shadow-primary-600/20">
                                {submitting || uploading ? <><Loader2 className="w-6 h-6 animate-spin" /> {uploading ? 'Uploading...' : 'Saving...'}</> : <><Upload className="w-6 h-6" /> {editingId ? 'Update Property' : 'Publish Now'}</>}
                            </button>
                        </form>
                    </div>
                </div>
