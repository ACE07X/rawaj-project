...
const Header = () => {
    ...
    return (
        <>
            <header
                className={`fixed w-full z-40 transition-all duration-500 ${isScrolled ? 'bg-slate-950/80 backdrop-blur-xl border-b border-white/10 py-4 shadow-2xl' : 'bg-transparent py-6'
                    }`}
            >
                    ...
                    <div className="absolute top-full left-0 w-full bg-slate-950/95 backdrop-blur-xl border-t border-white/10 p-4 md:hidden flex flex-col gap-4 animate-fade-in-down shadow-2xl">
                        ...
                        {user ? (
                            ...
                            <button
                                ...
                                className="w-full text-left px-4 py-3 rounded-xl bg-primary-600/20 text-primary-400 font-bold hover:bg-primary-600/30 transition-colors flex items-center gap-2"
                            >
                                <LogIn className="w-4 h-4" />
                                {lang === 'ar' ? 'تسجيل الدخول / إنشاء حساب' : 'Login / Join Us'}
                            </button>
                        )}
